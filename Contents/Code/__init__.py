...
def ShowList(sender, image, pageUrl):
    ...
    if clipdict['false']:
        dir.Append(Function(DirectoryItem(VideoList, title="Full Episodes", thumb=Function(GetThumb, url=image)), videolist=clipdict['false']))
    if clipdict['true']:
        dir.Append(Function(DirectoryItem(VideoList, title="Clips", thumb=Function(GetThumb, url=image)), videolist=clipdict['true']))
    return dir
